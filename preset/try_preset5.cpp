...
static void topo(int cur){
    ...
    while(!nodes.empty()){
        ...
        if(!in_deg[now])seq.push_back(now);
...
void make_package()
{
    ...
    */
    ...
    for(int node:seq){
        ...
        if(acc_weight + weight[node] > PKG_SIZE){
            if((tw + acc_weight) * 2 > 3 * PKG_SIZE)break;
            rst.push_back(cur_pkg);
            tw += acc_weight;
            acc_weight = weight[node];
            cur_pkg.clear();
            cur_pkg.push_back(node);
