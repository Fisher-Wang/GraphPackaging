...
int weight_depth[NODE_NUM_MAX];
int node_depth[NODE_NUM_MAX];
...
{
    node_depth[cur] = depth;
    weight_depth[depth] += weight[cur];
...
void make_package()
{
    ...
    for(bound = 0;bound < n;bound++){
        if(wnow + weight_depth[bound] <= TW / 2){
            wnow += weight_depth[bound];
        }
        else break;
    }
    for(int i = 1;i < n;i++){
        if(node_depth[i] <= bound){
            seq.push_back(i);
        }
    }
    int last_i = 0;
